console.log('Test 1: Script is starting!');
        ...
        async function getFsrsEngine() {
            if (!fsrsEnginePromise) {
                fsrsEnginePromise = (async () => {
                    ...
                    await engine.init();
                    if (globalSettings?.adaptiveFsrsBaseline) {
        ...
        function deriveImplicitFsrsRating(interactionLog, wasCorrect = true, providedBaseline = null) {
            const ratings = fsrsRatings || { Again: 0, Hard: 1, Good: 2, Easy: 3 };
            if (!wasCorrect) return ratings.Again;

            const baseline = providedBaseline || getFsrsBaseline();

            const latency = typeof interactionLog?.recallLatency === 'number'
                ? interactionLog.recallLatency
                : baseline.latency;
            const corrections = typeof interactionLog?.totalCorrections === 'number'
                ? interactionLog.totalCorrections
                : (interactionLog?.backspaceCount || 0) + (interactionLog?.deleteCount || 0);
            const attempts = typeof interactionLog?.attemptCount === 'number'
                ? interactionLog.attemptCount
                : interactionLog?.attempts || 1;
            const fluency = typeof interactionLog?.answerFluency === 'number'
                ? interactionLog.answerFluency
                : baseline.fluency;
        ...
        async function applyFsrsReviewUpdate(card, deckId, wasCorrect, interactionLog, iqs = 0.5) {
            ...
            let state = studyState?.knowledgeStates?.get(card.id)
                || await getDataFromDB('userKnowledgeState', ['default_user', card.id]);
            if (!state) state = createDefaultKnowledgeState(cardInDeck);
            state.recallHistory = state.recallHistory || [];
            if (!studyState.knowledgeStates) studyState.knowledgeStates = new Map();
            studyState.knowledgeStates.set(card.id, state);

            if (!state.fsrs) {
                const prepared = await prepareFsrsCard(cardInDeck);
                const snapshot = serializeFsrsCard(prepared);
                state.fsrs = snapshot;
                state.stability = prepared.stability ?? state.stability;
                const baseReviewDate = prepared.last_review
                    ? new Date(prepared.last_review)
                    : new Date();
                state.lastReviewed = baseReviewDate.toISOString();
            }

            const implicitBaseline = getFsrsBaseline();
            const oldStability = Number(state?.stability ?? state?.fsrs?.stability ?? 0) || 0;
            const rating = deriveImplicitFsrsRating(interactionLog, wasCorrect, implicitBaseline);
            const updatedCardState = await fsrsEngine.reviewCard(state.fsrs, rating);
            ...
            const updatedState = await updateKnowledgeState(
                card,
                wasCorrect,
                iqs,
                interactionLog?.questionType || 'Flashcard',
                updatedCardState?.stability ?? null,
                fsrsSnapshot,
                { rating, review: reviewTime }
            );

            if (updatedState) {
                updatedState.fsrs = fsrsSnapshot;
                updatedState.stability = updatedCardState?.stability ?? updatedState.stability;
                updatedState.stability = Number(updatedState.stability);
                updatedState.lastReviewed = reviewTime.toISOString();
                studyState.knowledgeStates?.set(card.id, updatedState);
            }

            if (globalSettings.devMode) {
                const finalStability = Number(updatedState?.stability ?? updatedCardState?.stability ?? state?.stability ?? 0) || 0;
                console.log('[FSRS debug] Adaptive Baseline:', implicitBaseline);
                console.log('[FSRS debug] Final Rating:', rating);
                console.log('[FSRS debug] FSRS Stability Before:', oldStability);
                console.log('[FSRS debug] FSRS Stability After:', finalStability);
            }

            return { fsrsSnapshot, rating, state: updatedState || state, reviewTime };
        }
        ...
        async function startLearnMode(deckId) {
            ...
            deck.cards.forEach(c => {
                ...
                if (state?.fsrs) c.fsrs = state.fsrs;
            });
            ...
            studyState.examDate = examDateObj || null;
            studyState.targetRetention = deck.settings.targetRetention || 0.8;
        ...
        async function updateSessionProgress() {
            ...
            studyState.sessionCardIds.forEach(id => {
                ...
                let score = hasExamDate ? (target ? retention / target : retention) : retention;
                if (score > 1) score = 1;
                totalScore += score;
            });
        ...
        async function updateLearnProgress() {
            ...
            allCards.forEach(card => {
                const state = knowledgeMap.get(card.id);
                const retention = calculateRetentionAtDate(state, targetDate);
                if (retention >= masteredThreshold) masteredCount++;
        ...
        async function continueStudy() {
            ...
            if (currentMode === 'learn') {
                ...
                const examDate = deckSettings.examDate ? new Date(deckSettings.examDate) : null;
                const targetRetention = deckSettings.examDate ? (deckSettings.targetRetention || 0.8) : 0.9;

                const nonMasteredCards = allCards.map(card => {
                    const state = knowledgeMap.get(card.id);
                    const retention = calculateRetentionAtDate(state, examDate || new Date());
                    return { ...card, deckId: currentDeckId, projectedRetention: retention };
                }).filter(card => card.projectedRetention < targetRetention);
            ...
            assignQuestionTypesToCards(studyState.roundCards);
        ...
        async function moveCard(card, correct, questionType = 'Flashcard') {
            ...
            if (currentMode === 'learn' || currentMode === 'exam') {
                ...
                const repeatResult = await fsrsEngine.repeat(preparedFsrsCard, new Date());
                const scheduled = repeatResult?.[implicitRating] || repeatResult?.[ratings.Good];
            ...
            if (currentMode === 'learn') {
                const updatedState = updatedKnowledgeState || await getDataFromDB('userKnowledgeState', ['default_user', card.id]);
                const deckSettings = deck.settings || {};
                const examDate = deckSettings.examDate ? new Date(deckSettings.examDate) : null;
                const targetRetention = examDate ? (deckSettings.targetRetention || 0.8) : 0.9;
                const retentionTargetDate = examDate || new Date();
                const retention = calculateRetentionAtDate(updatedState, retentionTargetDate);

                const streak = updatedState ? (updatedState.consecutiveCorrect || 0) : 0;
                const isMastered = retention >= targetRetention;
                ...
                if (isMastered) {
                    ...
                    showToast(examDate ? "Safe for the exam!" : "Card Mastered!", "success", 1000);
                } else {
        ...
        async function markSpaced(quality) {
            ...
            if (cardInDeck) {
                ...
                const ratings = fsrsRatings || fsrsEngine.getRatings();
                const updatedCardState = await fsrsEngine.reviewCard(cardInDeck.fsrs, fsrsRating);
                const fsrsSnapshot = serializeFsrsCard(updatedCardState);
                const reviewTime = updatedCardState?.last_review ? new Date(updatedCardState.last_review) : new Date();
                cardInDeck.fsrs = fsrsSnapshot;
        ...
        async function updateKnowledgeState(card, wasCorrect, iqs = 0.5, questionType = 'Flashcard', newStability = null, fsrsSnapshot = null, fsrsMeta = null) {
            ...
            return new Promise((resolve, reject) => {
                ...
                transaction.oncomplete = () => resolve(updatedState);
                ...
                request.onsuccess = () => {
                    ...
                    if (fsrsSnapshot) {
                        state.fsrs = fsrsSnapshot;
                        if (newStability === null && typeof fsrsSnapshot.stability === 'number') {
                            newStability = fsrsSnapshot.stability;
                        }
                    }

                    let stabilityToApply = newStability !== null ? Number(newStability) : null;
                    if (!Number.isFinite(stabilityToApply)) {
                        stabilityToApply = null;
                    }

                    if (stabilityToApply !== null) {
                        if (wasCorrect && prevStability !== null && stabilityToApply < prevStability) {
                            stabilityToApply = prevStability;
                        }
                        state.stability = stabilityToApply;
                    } else if (wasCorrect && state.stability < 100) {
                        const numericStability = Number(state.stability) || 0;
                        state.stability = numericStability < 100 ? numericStability * 1.1 : numericStability;
                    } else if (!wasCorrect) {
                        state.stability = 1.0; // Reset on failure
                    }

                    if (fsrsMeta?.review) {
                        state.lastReviewed = new Date(fsrsMeta.review).toISOString();
                    } else {
                        state.lastReviewed = new Date().toISOString();
                    }
                    state.lastModified = new Date().toISOString();
                    state.recallHistory.push({ date: state.lastReviewed, wasCorrect, iqs, rating: fsrsMeta?.rating || null });
        ...
        function calculateRetentionAtDate(state, targetDate) {
            ...
            const stability = (typeof state.stability === 'number' ? state.stability : null)
                ?? (typeof state.fsrs?.stability === 'number' ? state.fsrs.stability : null);
            const lastReviewedRaw = state.lastReviewed || state.fsrs?.last_review;
        ...
        function selectOptimalQuestionType(card, deckOverride = null, modeOverride = currentMode) {
            ...
            const state = knowledgeMap.get(card.id);
            const examDateSource = mode === 'exam'
                ? (studyState.examDate || resolvedDeck.settings.examDate)
                : resolvedDeck.settings.examDate;
            const examDate = examDateSource ? new Date(examDateSource) : null;

            const retentionExam = calculateRetentionAtDate(state, examDate || new Date());
            const normalizedRetention = Math.max(0, Math.min(1, retentionExam || 0));
            const difficulty = 1 - normalizedRetention;

            const answerText = (card.answer || '').trim();
            const answerTokens = answerText.length ? answerText.split(/\s+/).filter(Boolean) : [];
            const hasPunctuation = /[.,/#!$%^&*;:{}=\-_`~()'"?¿¡<>[\]\\|]/.test(answerText);
            const isShortTerm = answerTokens.length <= 3 && !hasPunctuation;
            const isSentence = hasPunctuation || answerTokens.length > 3;

            let scoreMCQ = difficulty * 0.9 + (isShortTerm ? 0.1 : 0);
            let scoreCloze = (1 - Math.abs(difficulty - 0.5)) + (isSentence ? 0.3 : 0);
            let scoreTyped = (1 - difficulty) * 1.1 + (isShortTerm ? 0.2 : 0);

            if (resolvedDeck.typeHint === 'Vocab' || isShortTerm) {
                scoreTyped += 0.35;
                scoreMCQ -= 0.25;
            }
            ...
            if (!adaptive.mcq || !canGenerateQuestionType('MultipleChoice', card, allCardsInDeck)) scoreMCQ = -Infinity;
            if (!adaptive.cloze || !canGenerateQuestionType('Cloze', card, allCardsInDeck)) scoreCloze = -Infinity;

            if (retentionExam > 0.65 && isShortTerm) {
                return 'Type';
            }

            const scoredTypes = [
                { type: 'MultipleChoice', score: scoreMCQ },
                { type: 'Cloze', score: scoreCloze },
                { type: 'Type', score: scoreTyped }
            ];

            scoredTypes.sort((a, b) => b.score - a.score);

            let finalType = scoredTypes[0].score === -Infinity ? 'Flashcard' : scoredTypes[0].type;

            if (scoredTypes[1] && scoredTypes[1].score > -Infinity && Math.random() < 0.10) {
                finalType = scoredTypes[1].type;
            }
        ...
        function updateCardDetailListForInsights(deckId, knowledgeMap) {
            ...
            deck.cards.forEach(card => {
                ...
                cardItem.innerHTML = `
                    ...
                    <div style="color:var(--secondary-text); font-weight:500;">Retention: ${retentionPercent}% | Stability: ${stability}d</div>
                `;
        ...
        function renderPlanAnalytics(plan, knowledgeMap) {
            ...
            hardestCardsList.innerHTML = sortedCards.map(card => `
                <div class="deck-card-item">
                    ...
                    <div style="color:var(--danger-color); font-weight:500;">${Math.round((card.projectedRetention ?? 0) * 100)}% Retention</div>
                </div>
            `).join('');
        }
        ...
        async function renderGlobalAnalytics() {
            try {
                ...
                // Render forecast graph placeholder
                const forecastCtx = document.getElementById('globalForecastGraphCanvas');
                if (forecastCtx) {
                    forecastCtx.innerHTML = '<p style="text-align: center; color: var(--secondary-text); padding: 40px;">Analytics visualization coming soon...</p>';
                }
